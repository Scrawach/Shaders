...
SubShader {
	...
	Pass {
		...
		struct pixel_t {
			...
			#endif
			fixed4 screenPosition	: TEXCOORD3;		
		};
		...
		pixel_t VertShader(vertex_t input)
		{
			...
			output.screenPosition = ComputeScreenPos(input.vertex);
		...
		{
			fixed time = _Time.y;
			fixed2 uv = saturate(smoothstep(-50, 50, input.screenPosition)).xy;
			fixed4 tex = tex2D(_NoiseTex, uv / 2);// + fixed2(0, -time / 20));
			fixed noise = tex.x; //smoothstep(-0.1, 1, tex);
