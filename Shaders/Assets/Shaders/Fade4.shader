    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {            	
                fixed time = _Time.y * _TimeScale;
                fixed2 uv = screenUV(input.uv);
                float2 flowVector = tex2D(_FlowMap, input.uv).rg;
                fixed flowNoise = tex2D(_FlowMap, input.uv).a;
                //fixed2 flow = flowUV(uv / 5, flowVector, time);
                //fixed4 noise = tex2D(_NoiseTex, flow);

            	fixed2 jump = fixed2(_UJump, _VJump);
				fixed3 uvwA = flowUVW(input.uv, flowVector, jump, _FlowOffset, _Tiling, time + flowNoise, false);
				fixed3 uvwB = flowUVW(input.uv, flowVector, jump, _FlowOffset, _Tiling, time + flowNoise, true);

                
                fixed4 noise_uwvA = tex2D(_NoiseTex, uvwA.xy  ) * uvwA.z;
                fixed4 noise_uwvB = tex2D(_NoiseTex, uvwB.xy  ) * uvwB.z;
                fixed4 noise = (noise_uwvA + noise_uwvB);


    // Time varying pixel color
    fixed4 col = tex2D(_NoiseTex, input.uv * 2 + time / 10);
    float grey = dot(col, fixed3(1,1,1)) / 3.0;    
    fixed3 img =  tex2D(_MainTex, input.uv);
    
    fixed2 uv0 = uv;
    float length0 = length(uv0);
    float time2 = _Progress * 50;
    fixed process = length0 * 20 - time2 * (grey * 0.5 + 0.5);        	
    float mask = 1.0 - clamp(process,0.0,1.0);
        
    // Output to screen
    fixed3 result_o = (1.0-mask) * img + mask;
	return mask * img.r;
